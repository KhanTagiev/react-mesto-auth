  ...
  return (
    <PopupWithForm title='Редактировать профиль' name='profile' isOpen={isOpen} onClose={onClose}
      ...
      <label className="form__label">
        <input className="form__input form__input_name" type="text" value={name} placeholder="Имя" name="profile-name"
               minLength="2" maxLength="40" required onChange={handleChangeName}/>
        <span className="form__input-error profile-name-error"></span>
      </label>
      <label className="form__label">
        <input className="form__input form__input_about" type="text" value={description} placeholder="О себе"
               name="profile-about" minLength="2" maxLength="200" required onChange={handleChangeDescription}/>
